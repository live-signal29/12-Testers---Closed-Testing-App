...
import { 
  ...
  Search, CheckCircle2, Flame, Info, LogIn, UserPlus, LogOut
} from 'lucide-react';

export default function App() {
  // Authentication & Navigation States
  const [isLoggedIn, setIsLoggedIn] = useState(false); // Default state Logged Out
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans antialiased pb-24">
      ...
      <header className="border-b border-slate-800/80 bg-slate-900/90 sticky top-0 z-50 px-4 py-3 flex justify-between items-center backdrop-blur-md">
        ...
        {/* Dynamic Header Actions (Coin Balance & Login/Logout) */}
          ...
          {isLoggedIn ? (
            ...
            <div className="flex items-center gap-1.5">
              <button 
                ...
                className={`text-xs font-bold px-3 py-1.5 rounded-xl transition ${authMode === 'login' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
              >
                Login
              </button>
              <button 
                onClick={() => setAuthMode('register')} 
                className={`text-xs font-bold px-3 py-1.5 rounded-xl transition ${authMode === 'register' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
              >
                Register
              </button>
            </div>
      ...
      <main className="max-w-md mx-auto w-full px-4 py-4 space-y-4 flex-1">
        ...
        {/* LOGIN / REGISTER SCREEN (SHOWS WHEN NOT LOGGED IN) */}
        {!isLoggedIn ? (
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-4 shadow-2xl mt-6">
            <div className="text-center space-y-1">
              <h2 className="text-lg font-black text-white">
                {authMode === 'login' ? 'Welcome Back Developer' : 'Create Developer Account'}
              </h2>
              <p className="text-xs text-slate-400">Join Google Group & Exchange App Testers</p>
            </div>
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-3 pt-2">
              ...
              <button 
                type="submit" 
                className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold py-3 rounded-xl text-xs shadow-lg transition"
              >
                {authMode === 'login' ? 'Login to Dashboard' : 'Register & Claim 100 Coins'}
              </button>
            </form>
      ...
      {/* BOTTOM NAVIGATION MENU BAR (ONLY VISIBLE WHEN LOGGED IN) */}
      {isLoggedIn && (
